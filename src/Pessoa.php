...
class Pessoa
{
  ...
  /**
   * (deprecated) Método para retornar estagiários ativos na unidade
   *
   * @param Integer $codundclgi
   * @return array
   * @deprecated em favor de listarEstagiarios, em 04/02/2026
   */
   ...
   * @return array
   */
  public static function listarEstagiarios($codundclg)
  {
    ...
    $query = "SELECT LOCALIZAPESSOA.*, PESSOA.* FROM LOCALIZAPESSOA
                    INNER JOIN PESSOA ON (LOCALIZAPESSOA.codpes = PESSOA.codpes)
                    WHERE ( LOCALIZAPESSOA.tipvin LIKE 'ESTAGIARIORH'
                        AND LOCALIZAPESSOA.codundclg IN (:codundclg)
                        AND LOCALIZAPESSOA.sitatl = 'A')
                    ORDER BY LOCALIZAPESSOA.nompes";
    $param = [
      'codundclg' => $codundclg,
